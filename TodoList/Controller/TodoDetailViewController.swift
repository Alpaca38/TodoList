...
//        }
        if let task = task {
            ...
            imageView.image = UIImage(named: "todoListImage")
            segmentedControl.selectedSegmentIndex = task.isCompleted ? 1 : 0
        }
    }
    ...
    private func showEditAlert() {
        ...
        alertController.addTextField { textField in textField.text = self.task?.title
        }
...
//                self?.todoItem?.dueDate = dueDate
                ...
                if let updatedItem = self?.task {
//                    TodoManager.shared.updateTodoItem(updatedItem)
                    updatedItem.title = title
                    updatedItem.modifyDate = Date()
                    TaskManager.shared.updateTask(newToDoData: updatedItem) {
...
//        CompletedManager.shared.deleteTodoItem(todoItem)
        TaskManager.shared.deleteTask(data: task) {
